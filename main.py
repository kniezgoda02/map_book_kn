...
class User:
    def __init__(self, name, surname, location, posts):
        ...
        self.marker = map_widget.set_marker(self.coordinates[0], self.coordinates[1],
                                            text="self.name, text='self.surname")


def get_coordinates(self: object) -> list:
    import requests
    from bs4 import BeautifulSoup
    adres_url: str = f'https://pl.wikipedia.org/wiki/{sefl.location}'
...
def update_user(i):
    ...
    users[i].coordinates = user[i].get_coordinates()
...
def show_user_detail():
    i = listbox_lista_obiektow.index(ACTIVE)
    label_szczegoly_obiektow_name_wartosc.config(text=users[i].name)
    label_szczegoly_obiektow_surname_wartosc.config(text=users[i].surname)
    label_szczegoly_obiektow_miejscowosc_wartosc.config(text=users[i].location)
    label_szczegoly_obiektow_posts_wartosc.config(text=users[i].posts)
...
button_pokaz_szczegoly = Button(ramka_lista_obiektow, text='Pokaż szczegóły:', command=show_user_detail)
...
label_szczegoly_obiektow_name = Label(ramka_szczegoly_obiektow, text='Imię:')
...
label_szczegoly_obiektow_surname = Label(ramka_szczegoly_obiektow, text='Nazwisko:')
label_szczegoly_obiektow_surname.grid(row=1, column=2)

label_szczegoly_obiektow_surname_wartosc = Label(ramka_szczegoly_obiektow, text='....:')
label_szczegoly_obiektow_surname_wartosc.grid(row=1, column=3)

label_szczegoly_obiektow_miejscowosc = Label(ramka_szczegoly_obiektow, text='Miejscowość:')
label_szczegoly_obiektow_miejscowosc.grid(row=1, column=4)

label_szczegoly_obiektow_miejscowosc_wartosc = Label(ramka_szczegoly_obiektow, text='....:')
label_szczegoly_obiektow_miejscowosc_wartosc.grid(row=1, column=5)

label_szczegoly_obiektow_posts = Label(ramka_szczegoly_obiektow, text='Posty:')
label_szczegoly_obiektow_posts.grid(row=1, column=6)

label_szczegoly_obiektow_posts_wartosc = Label(ramka_szczegoly_obiektow, text='....:')
label_szczegoly_obiektow_posts_wartosc.grid(row=1, column=7)
